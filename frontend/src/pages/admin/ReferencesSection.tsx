      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            ...
            <code className="bg-slate-100 text-slate-600 px-1 rounded font-mono text-[11px]">
              {"{{ref:key_name}}"}
            </code>{" "}
      ...
      <div className="rounded-xl border border-slate-200 overflow-hidden">
        {!hasRows ? (
          <div className="py-10 text-center text-slate-400 text-sm">
            No references yet. Create one to start using{" "}
            <code>{"{{ref:key_name}}"}</code> in your content.
          </div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {systemItems.map((item) => [
                <tr key={`sys_${item.key}`} className="border-b border-slate-100 bg-slate-50/60 hover:bg-slate-100/60 transition-colors">
                  <tr className="border-b border-slate-100 bg-slate-50/60 hover:bg-slate-100/60 transition-colors">
                    <td className="px-4 py-3">
                      <span className="font-medium text-slate-800">
                        {formatSystemLabel(item.key, item.description)}
                      </span>
                      <span className="ml-2 text-[10px] text-slate-400 bg-slate-200 px-1.5 py-0.5 rounded">
                        system
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      ...
                      <td colSpan={5} className="px-4 py-4">
                        <div className="flex items-center gap-3">
                          <div className="flex-1">
                            ...
                            <Input
                              value={systemDraftValue}
                              onChange={(e) => setSystemDraftValue(e.target.value)}
                              placeholder="https://..."
              ...
              {refs.map((ref) => (
                <Fragment key={ref.id}>
                  <tr className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => copyKey(ref.key)}
                        title="Click to copy reference syntax"
                    ...
                    <td className="px-4 py-3 text-slate-600 max-w-xs truncate text-xs">
                      {ref.type === "email" ? (
                        <a href={`mailto:${ref.value}`} className="text-blue-600 hover:underline">
                          {ref.value}
                        </a>
                      ) : ref.type === "url" ? (
                        <a
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          data-testid={`delete-ref-${ref.key}`}
                        >
                          <Trash2 size={13} />
                        </button>
                      </div>
    ...
    <div className="grid grid-cols-2 gap-3">
      ...
      <div>
        ...
        <Input
          ...
          type={
            draft.type === "email" ? "email" : draft.type === "url" ? "url" : "text"
          }
          className="mt-0.5 h-8 text-sm"
      ...
      <div className="col-span-2">
        ...
        <Input
          ...
          placeholder="Short description of this reference"
          className="mt-0.5 h-8 text-sm"
          data-testid="ref-description-input"
        />
      </div>
    </div>
  );
