                ...
                <TableBody>
                  ...
                  {paged.map((product) => (
                    <TableRow key={product.id} data-testid={`admin-product-row-${product.id}`}>
                      ...
                      <TableCell className="text-sm">{product.base_price ? new Intl.NumberFormat("en-US", { style: "currency", currency: product.currency || "USD", minimumFractionDigits: 0, maximumFractionDigits: 2 }).format(product.base_price) : product.pricing_type === "enquiry" ? <span className="text-slate-400 text-xs">RFQ</span> : <span className="text-slate-400 text-xs">—</span>}</TableCell>
                      <TableCell>
