...
export function OverrideCodesTab() {
  ...
  return (
    <div className="space-y-4" data-testid="override-codes-tab">
              ...
              <TableRow key={oc.id} data-testid={`admin-oc-row-${oc.id}`} className="border-b border-slate-100">
                ...
                <TableCell>
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="sm" className="h-6 px-2 text-[11px]" onClick={() => { setLogsUrl(`/admin/override-codes/${oc.id}/logs`); setShowAuditLogs(true); }} data-testid={`admin-oc-logs-${oc.id}`}>Logs</Button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
      ...
      <AuditLogDialog open={showAuditLogs} onOpenChange={setShowAuditLogs} title="Override Code Audit Logs" logsUrl={logsUrl} />
      <ImportModal
