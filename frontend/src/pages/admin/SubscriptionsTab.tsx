...
export function SubscriptionsTab() {
  ...
  const uniqueSubNumbers = Array.from(new Set(subs.map(s => s.subscription_number || s.id?.slice(0, 8)).filter(Boolean)));  // M-10: deduplicated
  ...
  useEffect(() => {
    ...
    api.get("/admin/products-all?per_page=500").then(r => {
      ...
      setProducts(allProds.filter((p: any) => !p.deleted_at && p.is_active !== false && p.is_subscription === true));
    }).catch(() => {});
        ...
        <DialogContent><DialogHeader><DialogTitle>Create Manual Subscription</DialogTitle></DialogHeader>
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <RequiredLabel className="text-slate-500 font-normal">Billing Interval</RequiredLabel>
