...
export function ArticleTemplatesTab({ categories }: { categories?: any[] }) {
  ...
  const downloadCsv = () => {
    ...
    fetch(`${base}/api/admin/export/resource-templates`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.blob()).then(b => { const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = `resource-templates-${new Date().toISOString().slice(0,10)}.csv`; a.click(); })
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await api.put(`/resource-templates/${editing.id}`, form);
        toast.success("Template updated");
      } else {
        await api.post("/resource-templates", form);
    ...
    <div className="space-y-4" data-testid="resource-templates-tab">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" onClick={downloadCsv} data-testid="resource-templates-export-csv"><Download size={14} className="mr-1" />Export CSV</Button>
          <Button size="sm" variant="outline" onClick={() => setShowImport(true)} data-testid="resource-templates-import-csv"><Upload size={14} className="mr-1" />Import CSV</Button>
              ...
              <TableRow key={tpl.id} data-testid={`template-row-${tpl.id}`}>
                ...
                <TableCell>
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="sm" className="h-7 px-2 text-[11px]" onClick={() => { setLogsUrl(`/resource-templates/${tpl.id}/logs`); setShowAuditLogs(true); }} data-testid={`template-logs-${tpl.id}`}><Clock size={11} /></Button>
                    <Button variant="ghost" size="sm" className="h-7 px-2 text-[11px] text-red-500 hover:text-red-700" onClick={() => setConfirmDeleteTpl(tpl.id)} disabled={deleting === tpl.id} data-testid={`template-delete-${tpl.id}`}>
                      <Trash2 size={11} />
                    </Button>
                  </div>
                </TableCell>
      ...
      <ImportModal
        ...
        entityLabel="Resource Templates"
        open={showImport}
      ...
      {/* Logs Dialog */}
      <AuditLogDialog open={showAuditLogs} onOpenChange={setShowAuditLogs} title="Template Logs" logsUrl={logsUrl} />
      ...
      <AlertDialog open={!!confirmDeleteTpl} onOpenChange={(open) => !open && setConfirmDeleteTpl(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction className="bg-red-600 hover:bg-red-700" onClick={() => { handleDelete(confirmDeleteTpl!); setConfirmDeleteTpl(null); }} data-testid="confirm-resource-tpl-delete">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
