...
export function ArticleCategoriesTab() {
  ...
  const downloadCsv = () => {
    ...
    fetch(`${base}/api/admin/export/resource-categories`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.blob()).then(b => { const a = document.createElement("a"); a.href = URL.createObjectURL(b); a.download = `resource-categories-${new Date().toISOString().slice(0,10)}.csv`; a.click(); })
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await api.put(`/resource-categories/${editing.id}`, form);
        toast.success("Category updated");
      } else {
        await api.post("/resource-categories", form);
  ...
  const handleDelete = async (cat: Category) => {
    setDeleting(cat.id);
    try {
      await api.delete(`/resource-categories/${cat.id}`);
  ...
  return (
    <div className="space-y-4" data-testid="resource-categories-tab">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {categories.length} categor{categories.length !== 1 ? "ies" : "y"} — used to organise resources on the store
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" onClick={downloadCsv} data-testid="resource-categories-export-csv"><Download size={14} className="mr-1" />Export CSV</Button>
          <Button size="sm" variant="outline" onClick={() => setShowImport(true)} data-testid="resource-categories-import-csv"><Upload size={14} className="mr-1" />Import CSV</Button>
              ...
              <TableRow key={cat.id} data-testid={`category-row-${cat.id}`}>
                ...
                <TableCell>
                  <div className="flex gap-1">
                    ...
                    <Button variant="ghost" size="sm" className="h-7 px-2 text-[11px]" onClick={() => { setLogsUrl(`/resource-categories/${cat.id}/logs`); setShowAuditLogs(true); }} data-testid={`category-logs-${cat.id}`}>
                      <Clock size={11} />
                    </Button>
          ...
          <div className="space-y-4 py-1">
            ...
            <label className="flex items-center gap-2 cursor-pointer select-none">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">Resources in this category require a price to be set</p>
              </div>
            </label>
      ...
      <ImportModal
        ...
        entityLabel="Resource Categories"
        open={showImport}
      ...
      {/* Logs Dialog */}
      <AuditLogDialog open={showAuditLogs} onOpenChange={setShowAuditLogs} title="Category Logs" logsUrl={logsUrl} />
      ...
      <AlertDialog open={!!confirmDeleteCat} onOpenChange={(open) => !open && setConfirmDeleteCat(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Are you sure you want to delete "{confirmDeleteCat?.name}"? Resources using this category will need to be reassigned.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction className="bg-red-600 hover:bg-red-700" onClick={() => { handleDelete(confirmDeleteCat!); setConfirmDeleteCat(null); }} data-testid="confirm-resource-cat-delete">
              Delete
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
