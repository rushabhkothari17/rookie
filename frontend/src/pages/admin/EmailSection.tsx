        ...
        <div className="p-5 overflow-y-auto flex-1 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              {subject.length > 0 && (
                <span className={`text-[11px] font-mono tabular-nums ${subject.length > 95 ? "text-red-500" : subject.length > 80 ? "text-amber-500" : "text-slate-400"}`}>{subject.length}/100</span>
              )}
            </div>
            <Input value={subject} onChange={e => setSubject(e.target.value)} maxLength={100} className="mt-1" data-testid="template-subject-input" />
          </div>
