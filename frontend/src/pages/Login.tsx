    ...
    <div className="min-h-screen grid place-items-center bg-slate-50 grid-rhythm" data-testid="login-page">
      <div className="glass-card w-full max-w-md rounded-2xl p-8">
        <div className="space-y-2">
          <p className="text-xs uppercase tracking-[0.2em] text-slate-400" data-testid="login-portal-label">
            {ws.login_portal_label || "Customer Portal"}
          </p>
          <h1 className="text-3xl font-semibold text-slate-900" data-testid="login-title">
            {ws.login_title || "Welcome back"}
          </h1>
          <p className="text-sm text-slate-500" data-testid="login-subtitle">
            {ws.login_subtitle || "Sign in to continue."}
          </p>
        </div>
        <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
          <div className="space-y-2">
            <label className="text-sm text-slate-600" htmlFor="email">Email</label>
            <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} data-testid="login-email-input" required />
          </div>
          <div className="space-y-2">
        ...
        <div className="mt-6 text-sm text-slate-500">
          New here?{" "}
          <Link to="/signup" className="text-blue-600" data-testid="login-signup-link">Create an account</Link>
        </div>
      </div>
    </div>
  );
