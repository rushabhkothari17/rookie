...
def admin_session():
    """Login as tenant admin (no partner_code) and return session with auth headers."""
...
class TestCRMMappingsNoQuoteRequests:
    ...
    def test_crm_mappings_returns_200(self, admin_session):
        """GET /api/admin/integrations/crm-mappings should return 200."""
        resp = admin_session.get(f"{BASE_URL}/api/admin/integrations/crm-mappings")
        assert resp.status_code == 200, f"Expected 200, got {resp.status_code}: {resp.text}"
        print("PASS: /api/admin/integrations/crm-mappings returns 200")

    def test_webapp_modules_structure(self, admin_session):
        """Response must include webapp_modules key with a list."""
        resp = admin_session.get(f"{BASE_URL}/api/admin/integrations/crm-mappings")
        data = resp.json()
        assert "webapp_modules" in data, f"Missing webapp_modules in response: {list(data.keys())}"
    ...
    def test_webapp_modules_no_quote_requests(self, admin_session):
    ...
    def test_webapp_modules_contains_expected_modules(self, admin_session):
...
class TestImportsQuoteRequestsRemoved:
    ...
    def test_import_quote_requests_returns_400(self, admin_session):
        ...
        session_no_ct = requests.Session()
        session_no_ct.headers.update({"Authorization": admin_session.headers.get("Authorization", "")})
        resp = session_no_ct.post(
    ...
    def test_import_quote_requests_error_message(self, admin_session):
    ...
    def test_import_override_codes_returns_400(self, admin_session):
    ...
    def test_import_valid_entity_still_works(self, admin_session):
        """POST /api/admin/import/categories with valid CSV should work (not 400 for unknown entity)."""
...
class TestEntityCollectionsIntegrity:
    ...
    def test_valid_entities_recognized(self, admin_session, entity):
        """Valid entities should NOT get 'Unknown entity' 400 error."""
        # Minimal CSV with just headers, no data rows
