...
def get_admin_token(partner_code="automate-accounts"):
    """Get admin token using partner-login (tenant admin)"""
    ...
    if resp.status_code == 200:
        return resp.cookies.get("aa_token") or resp.json().get("token")
    return None

def get_platform_admin_token():
    """Get platform admin token using /auth/login"""
...
def get_customer_token(partner_code="automate-accounts"):
    """Get customer token using customer-login"""
