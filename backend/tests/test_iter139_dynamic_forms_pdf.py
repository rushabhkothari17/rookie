...
class TestFormsCRUD:
    ...
    def test_get_forms_returns_list(self, auth_headers):
    ...
    def test_create_form_success(self, auth_headers):
    ...
    def test_create_form_missing_name(self, auth_headers):
    ...
    def test_create_and_update_form(self, auth_headers):
        """Create a form then PUT to update it."""
        # Create
        create_resp = requests.post(
            f"{BASE_URL}/api/admin/forms",
            json={"name": "TEST_UpdateMe", "form_schema": "[]"},
        ...
        update_resp = requests.put(
            ...
            json={"name": "TEST_UpdateMe-Renamed", "form_schema": '[{"type":"text","label":"Name"}]'},
            headers=auth_headers,
        )
        assert update_resp.status_code == 200, f"Expected 200, got {update_resp.status_code}: {update_resp.text}"
        update_data = update_resp.json()
        assert "message" in update_data or "form" in update_data, f"Unexpected response: {update_data}"
        ...
        assert updated["name"] == "TEST_UpdateMe-Renamed", f"Name not updated: {updated}"
        print(f"PASS: Updated form name confirmed via GET list")

        # Cleanup
        del_resp = requests.delete(f"{BASE_URL}/api/admin/forms/{form_id}", headers=auth_headers)
        assert del_resp.status_code == 200, f"Delete failed: {del_resp.text}"
        print(f"PASS: Cleanup - deleted form {form_id}")

    def test_update_nonexistent_form(self, auth_headers):
    ...
    def test_create_and_delete_form(self, auth_headers):
        """Create a form then DELETE it."""
        # Create
        create_resp = requests.post(
            f"{BASE_URL}/api/admin/forms",
            json={"name": "TEST_DeleteMe", "form_schema": "[]"},
            headers=auth_headers,
        )
        assert create_resp.status_code == 200
        form_id = create_resp.json()["form"]["id"]
        ...
        # Verify deletion via GET list
        list_resp = requests.get(f"{BASE_URL}/api/admin/forms", headers=auth_headers)
        forms = list_resp.json().get("forms", [])
        assert not any(f["id"] == form_id for f in forms), "Deleted form should not appear in list"
        print(f"PASS: Deleted form confirmed absent from GET list")

    def test_delete_nonexistent_form(self, auth_headers):
...
class TestEnquiryPDF:
    """Tests for the enquiry PDF endpoint."""

    def test_pdf_endpoint_nonexistent_order(self, auth_headers):
    ...
    def test_pdf_endpoint_unauthenticated(self):
...
class TestFormsCleanup:
    """Cleanup any TEST_ forms left over."""

    def test_cleanup_test_forms(self, auth_headers):
