...
from core.tenant import get_tenant_filter, tenant_id_of, DEFAULT_TENANT_ID, get_tenant_admin, resolve_api_key_tenant, is_platform_admin, enrich_partner_codes
...
async def list_articles_admin(
    ...
    if partner and is_platform_admin(admin):
        # Find tenant_ids for the given partner codes
        partner_codes = [p.strip() for p in partner.split(",") if p.strip()]
        tenants = await db.tenants.find({"code": {"$in": partner_codes}}, {"_id": 0, "id": 1}).to_list(100)
...
async def list_articles_public(
    ...
    if user and is_platform_admin(user):
        tid = None  # no filter — platform admin sees all resources
...
async def get_article_by_id(
    ...
    # Platform admins search across all tenants; others use own tenant_id
    if is_platform_admin(user):
        tid = None  # No filter — search all tenants
    else:
        tid = user.get("tenant_id") or DEFAULT_TENANT_ID
...
async def update_article(
    ...
    if changes:
        ...
        await create_audit_log(entity_type="resource", entity_id=resource_id, action="updated", actor=admin.get("email", "admin"), details=changes)
...
async def delete_article(
    ...
    await create_audit_log(entity_type="resource", entity_id=resource_id, action="deleted", actor=admin.get("email", "admin"), details={"title": article.get("title")})
